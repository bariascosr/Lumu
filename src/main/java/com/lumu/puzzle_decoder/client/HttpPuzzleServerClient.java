...
public class HttpPuzzleServerClient implements PuzzleServerClient {
    ...
    private Mono<PuzzleFragment> processResponse(
            HttpResponse<String> response) {

        if (response.statusCode() < 200 || response.statusCode() >= 300) {

            return Mono.error(
                    new FragmentClientException(
    ...
    private FragmentResponseDto deserialize(String body) {
        try {
            return objectMapper.readValue(
                    body,
                    FragmentResponseDto.class);
        } catch (Exception exception) {
            throw new FragmentClientException(
